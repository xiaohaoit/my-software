...
def ImageDailyPaper():
	...
	cur_x, cur_y = 10, 10
	# 早报
	cur_x, cur_y = image_opt.drawText(text=f"{date} [ {week} ] 潭州教育南昌分公司晨报", x=cur_x, y=cur_y + 5, font=ImageFont.truetype("./Resource/Fonts/simkai.ttf", 24))
#    cur_x, cur_y = image_opt.drawText(text=f"潭州教育南昌分公司晨报1", x=cur_x, y=cur_y + 5)
	# 天气预报
	if weather_ls:
...
#       image_opt.drawImage(image=image_opt.resizeImage(image=LoadImage(image_url1=f"http://chuantu.xyz/t6/712/1579007797x977013264.png"), x=460, y=60, show_mask=True)
...
#	if tanzhou_ls:
#		cur_x, cur_y = image_opt.drawImage(image=Image.open(f'./Resource/Skins/Default/bg-0-journalism.png'), x=cur_x, y=cur_y + 10)
#		for idx, tanzhou in enumerate(tanzhou_ls, start=1):
#			cur_x, cur_y = image_opt.drawText(text=f"{idx}、{tanzhou['title']}:{tanzhou['desc']}", x=cur_x, y=cur_y + 5, maxWidth=33)	
    # 心语
	if sentence_ls:
		...
		for idx, sentence in enumerate(sentence_ls, start=1):
			cur_x, cur_y = image_opt.drawText(text=f"{sentence['content']} —— {sentence['mrname']}  潭州小伙伴们加油,冲冲冲！", x=cur_x, y=cur_y + 5, maxWidth=33)
           # cur_x, cur_y = image_opt.drawText(text=f" 潭州教育南昌分公司晨报", x=cur_x, y=cur_y + 5, maxWidth=33)
 
#    cur_x,cur_y = image_opt_drawText(text=f"sssss!", x=cur_x, y=cur_y + 10)
...
#def TextDailyPaper():
#	daily_lines = list()
#	daily_lines.append(f"{date} [ {week} ] 潭州教育南昌分公司晨报")
#	if weather_ls:
...
# 邮件日报
#def MailDailyPaper():
#	sendMail(subject=f"{date} [{week}] 潭州教育南昌分公司晨报", contents=TextDailyPaper(), attachments=ImageDailyPaper())
